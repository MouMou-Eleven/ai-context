...
> 写给以后再为秒哒做可控技能包的人。本文重点覆盖自行开发或导入 `.zip` 技能包的实战规则；V3.5 新增的对话创建、`SKILL.md` 导入和 API 文档创建入口见第九节。所有包结构与校验规则都来自真实上传结果，并与官方资料对照过。
...
## 九、V3.2 后对话创建，V3.5 扩展技能入口

> 来源：百度秒哒 26 年官方更新日志，2026.06.24 — V3.2；[秒哒 3.5 官方发布文章](https://mp.weixin.qq.com/s/y2_ip4MYlJQPpTJ2a_O26A?scene=1)，2026.07.16。

V3.2 上线后，秒哒支持在任意项目中通过对话生成、编辑专属技能。V3.5 进一步明确了 4 类入口：

1. 首页选择「技能创建」项目类型，用一句话生成专属技能。
2. 在任何项目的对话中提出需求，或 `@` 已有技能进行修改。
3. 导入现有 `.zip` 技能包或单个 `SKILL.md` 文件。
4. 粘贴第三方 API 文档链接或 API 说明，让秒哒解析并生成可调用技能。

生成后的技能可保存到「技能中心」，在任意项目用 `@技能名` 复用。V3.5 的第 4 类入口取代了 2026-05-10 文档中“API 文档链接/说明创建已暂时下架”的限制。

但它不改变本文件前面的底层规则：

- **SKILL.md 仍要符合校验**：特别是 YAML frontmatter、`name`、`description`、`license` 等字段
- **密钥仍只能放环境变量**：不能让对话生成的技能把真实 key 写进 SKILL.md、脚本或参考资料
- **复杂技能仍要做静态验证**：涉及签名、支付、上传、外部 API、批量数据处理时，仍建议本地生成 zip、跑完静态检查后再上传
- **对话生成后要反查结构**：要求秒哒回贴技能目录结构或生成结果、入口/调用方式、环境变量名、入参/出参格式，确认没有遗漏
