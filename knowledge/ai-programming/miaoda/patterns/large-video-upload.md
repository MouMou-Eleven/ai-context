# 大视频上传：分片不合并 + HTTP Range 流式代理

> 这是 [pitfalls.md #14](../pitfalls.md) 在生产已验证的处方。本文档只回答**怎么落地**，不再讲叙事。
>
> 三个函数的可直接抄走的源码：[`references/video-chunked-upload/`](../../../../references/video-chunked-upload/)（不要照症状重写，照抄即可）。
...
| 场景 | 这个方案 |
|---|---|
| 秒哒 / Supabase 后端 + 浏览器上传单文件 ≥ 50MB（视频 / 大图集 / PDF）→ 撞 CORS / 413 / supervisor kill | ✅ 用 |
