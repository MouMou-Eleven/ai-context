...
>
> 旧环境兼容源码：[`references/video-chunked-upload/`](../../../../references/video-chunked-upload/)；只有当前环境仍复现 CORS / 413 / supervisor kill 时才使用。
...
先让秒哒只做能力测试并停止，不要一边测试一边改架构。确认仍复现旧限制后，再把 `references/video-chunked-upload/` 三个 ts 作为兼容源码喂给它，避免它重写后再次撞 #14 的旧环境三层墙。
