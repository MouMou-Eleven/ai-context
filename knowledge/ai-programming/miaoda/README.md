...
| [pitfalls.md](./pitfalls.md) | 20 个真实踩坑清单：MD5 Web Crypto 死路、密钥泄漏、嵌套目录、AI 自动重写签名、无 Supabase 控制台、多通道退款误路由、**大视频上传三层墙**、**SPA SEO 漏 keywords**、**浏览器缓存伪存储**、**应用形态强转**、**小程序上传 invalid filepath**、**登录自写 Edge Function**等 | 遇到怪异报错、或想预防性避雷时 |
...
- 重复实施 ≥ 2 次的复杂方案，沉淀为 `patterns/<name>.md`（处方）+ 配套的可抄源码到 `references/<name>/`
