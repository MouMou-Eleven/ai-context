...
```text
python -B repository/maintenance/context-route.py --task "做MG动画微课，用AI生成镜头" --intent create
python -B repository/maintenance/sync-navigation.py
python -B repository/maintenance/sync-structure.py
python -B repository/maintenance/validate-context.py
python -B -m unittest discover -s repository/maintenance/tests -v
```
...
| [sync-navigation.py](./sync-navigation.py) | 从注册表生成 llms 与个人业务中的项目入口区块，支持只读漂移检查 |
| [sync-structure.py](./sync-structure.py) | 从实际非忽略文件树生成完整 STRUCTURE.md 和交互 HTML；保留有效中文描述，清除失效描述 |
...
| [validation-policy.json](./validation-policy.json) | 保留原 PowerShell 校验器的50项必需文件、29个目录边界和废弃路径检查 |
...
- 实际目录树以 Git 跟踪文件及未忽略的新文件为准，排除 `.git`、被忽略的临时文件和缓存。完整上游快照在结构树折叠展示，内部数量仍按真实文件计算。
...
- 原50项入口、29个领域目录、旧路径、跨领域边界、敏感线索和重复内容提示继续检查。旧“所有中文任务强制预读长文件”断言改为短入口与注册表路径检查，符合当前渐进读取规则。
...
本地启用：`git config core.hooksPath repository/maintenance/git-hooks`。提交前检查导出**整个暂存区**，包含暂存的规则、脚本与生成物；未暂存修复不能遮住暂存区错误，未暂存新文件也不会被顺手提交。生成内容引用了未暂存的新文件或输入时，暂存快照会缺链接或出现漂移，提交被阻止。
...
Git hooks中的桌面同步只复制 `STRUCTURE.html`，不重写仓库生成物；Windows桌面不可用时记录延后，后续Git操作再次尝试。手动兼容命令 `powershell -ExecutionPolicy Bypass -File repository/maintenance/sync-desktop-structure.ps1` 会显式生成再同步；加 `-Strict` 可将桌面不可用视为错误。
