# 飞书高效办公：AI时代职场人的效率翻倍实战指南

> 建委著 | 状态：进行中
> 当前目录口径确认：2026-06-26
> 最近项目记录：2026-08-10
> 索引最后整理：2026-08-10

## 概述

一本面向普通职场人的 AI 办公实战书，核心定位：**大道至简——用最接地气的话，讲最不简单的事。**

- **目标读者**：非技术背景的职场人，日常工作中涉及文档、表格、PPT、数据汇报
- **差异化**：设计师视角（不是程序员教AI）、真实培训验证（线下讲过）、变现闭环（市面上极少）
- **当前进度**：第2章已完成编辑反馈、括号序号纠正和 Word 清洁交付版；第3章已完成编辑反馈、图3-1最终口径和 Word 交付版；第8章已对最新编辑反馈进行二次全量复核，38个修订段落和35条批注全部通过独立验收，飞书正文更新至 `revision_id=271`，19页Word交付终稿已重新生成并逐页校验。

## 写作铁律（所有 AI 工具必须遵守）

> ⚠️ **绝对禁止凭经验推断、臆想、猜测或虚构飞书功能。**

- 所有飞书功能描述必须基于飞书官方文档（feishu.cn/hc）或官方产品页验证
- 操作步骤中的菜单名称、按钮名称必须与飞书实际界面完全一致
- 如果无法从官方渠道确认某个功能是否存在，宁可不写，也不要编造
- 飞书产品迭代快，写作前必须查阅最新官方文档，不要依赖过往记忆
- 参考来源：feishu.cn/hc（帮助中心）、feishu.cn/product（产品页）、open.feishu.cn（开放平台）

## 正文与素材清单分工（重要）

- **正文写给读者**：图前引导只说明这张图能帮助读者看什么，图后解释只讲图中的关键点、操作意义和工作方法。
- **素材清单写给作者**：待截图、截图角度、画面应展示哪些区域、哪些内容不要截，统一放到正文下方的素材清单或截图清单。
- **禁止把作者提示写进正文**：例如“妙记页面应重点展示 AI 总结和章节分段，而不是只展示一大段逐字稿”这类话，是给作者或截图制作者看的，不能进入正文。正文应改成读者视角，例如说明读者从图中能看到会议内容如何被整理成总结、章节和待办。
- **禁止把资料搜集过程写进正文**：“飞书帮助中心列出的……”“根据官方资料……”属于核验或来源记录，应放素材清单、修订记录或参考文献；正文直接说明经过核验的事实、方法和适用边界。

## 软件界面截图与 Word 交稿规则（重要）

- **截图先服务印刷阅读**：不要为了完整而放整屏小字截图，优先截核心区域，保证导入 Word 后关键文字仍能看清。
- **图文必须真实对应**：正文写“如图4-x所示”前，必须实际查看图片，确认图片内容、红框、箭头、圈号与正文解释一一对应。
- **图片替换要同步修正文案**：替换截图后必须补回或更新图题、caption、正文引用和图号表，不能让图片和文字各说各的。
- **详见规范文件**：`interface-screenshot-guidelines.md` 是后续界面截图、改图、Word 交稿前检查的必读文件。

## 当前全书目录（2026-06-26 确认）

最新目录来源：飞书文档《飞书书籍·目录（第二版）- 标题更新版》，wiki 节点 `CmiXwFmaCiejwEkegnBc9d6an9g`，底层文档 `GByFdVLr7odeW1xX5JlcQNvJn7b`，读取版本 `revision_id=79`。

| 章节 | 作者 | 状态 | 飞书文档链接 |
|------|------|------|-------------|
| 前言 | 待确认 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 第1章 飞书协作模式：AI时代的高效工作方式 | 另一位作者 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 第2章 晨间工作梳理：用消息分组和飞书 aily 整理任务 | 建委 | 编辑反馈、括号序号与Word交付版已完成 | [正文](https://my.feishu.cn/wiki/HBdBwRwcTikxd8k6J4ScIHMEn7c) / [截图清单](https://my.feishu.cn/wiki/XK2CwerKFiwXOrkh6OLcU2Jwnbd) |
| 第3章 方案撰写实战：用模板、AI与评论协作完成文档 | 建委 | 编辑反馈、图3-1最终口径和Word交付版已完成 | [正文](https://my.feishu.cn/wiki/QjUfwODpLipOg6kKZ96c8n5YnSQ) / [截图清单](https://my.feishu.cn/wiki/APJKw9CtwigBP1kfBXYcL5k8nKd) |
| 第4章 会议闭环管理：用妙记与任务推进会后跟进 | 待确认 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 第5章 内容排期管理：用多维表格实现任务流转 | 另一位作者 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 第6章 项目从启动到复盘：飞书的全流程协作实践 | 待确认 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 第7章 客户跟进管理：用多维表格搭建轻量业务系统 | 待确认 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 第8章 重复工作自动化：用自动化与 AI 工作流提升效率 | 建委 | 编辑反馈二次全量复核完成；飞书 `revision_id=271`，38/38个修订段和35/35条批注通过，19页Word终稿已重建 | [正文](https://my.feishu.cn/wiki/UZyHw8RFCicH30kihDGcFJTZnPf) / [截图清单](https://my.feishu.cn/wiki/FzmVwHZJPiVb9EkNSgrcH1fRnKh) |
| 第9章 AI操作飞书：智能伙伴、Agent与CLI | 待确认 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |
| 附录（可选） | 待确认 | 目录确认 | [目录文档](https://eadcgzghjzt.feishu.cn/wiki/CmiXwFmaCiejwEkegnBc9d6an9g) |

**章节边界约定**：
- 建委当前主要负责：第2章、第3章、第8章；当前优先处理第2章、第3章。
- 旧第4章《普通职场人如何用飞书每天省下2小时》不再作为当前目录下的完整章结构使用，而是作为已审阅素材来源。
- 旧 4.1、4.2 主要进入新版第2章；旧 4.3 主要进入新版第3章；旧 4.4 对应新版第4章；旧 4.5 更接近新版第8章。
- 第2章聚焦晨间个人工作梳理：消息标签、置顶管理、消息转任务、任务中心、飞书 aily。
- 第3章聚焦方案撰写：模板、AI起草、评论协作、历史记录、审批/知识库归档。

**避免重复原则**：后续写作必须以最新目录为准，旧第4章只能作为素材库使用，不能继续沿用旧章号、旧图号或旧章节边界。

## 章节进度

| 章节 | 状态 | 字数 | 存放位置 | 完成日期 |
|------|------|------|----------|----------|
| 旧第4章 每天被琐事淹没：普通职场人如何用飞书每天省下2小时 | ✅ 历史素材，已审阅多轮 | ~16,000字 | 飞书文档 | 2026-05-11 |
| 第2章 晨间工作梳理：用消息分组和飞书 aily 整理任务 | ✅ 编辑反馈、括号序号、XML校验与Word交付版已完成 | 待统计 | 飞书文档 + F盘桌面Word | 2026-07-14 |
| 第3章 方案撰写实战：用模板、AI与评论协作完成文档 | ✅ 编辑反馈、图文、序号、官方核验与Word交付版已完成 | 待统计 | 飞书文档 + F盘桌面Word | 2026-07-18 |
| 第8章 重复工作自动化：用自动化与 AI 工作流提升效率 | ✅ 编辑反馈二次全量复核、遗漏纠正、飞书第271版和Word重建已完成 | 约0.97万中文字符 | 飞书文档 + F盘桌面Word + 素材 Base | 2026-08-10 |

## 文件索引

| 文件 | 内容 | 何时读取 |
|-----|------|---------|
| [`revisions/2026-08-10-ch8-editor-feedback-full-reaudit-correction.md`](./revisions/2026-08-10-ch8-editor-feedback-full-reaudit-correction.md) | 第8章编辑反馈遗漏根因、13段重新合并、38/38修订和35/35批注验收、飞书第271版及Word重建结果 | **继续修改第8章、处理任何编辑反馈或重新生成Word前优先读取** |
| [`revisions/2026-08-10-ch8-editor-feedback-applied.md`](./revisions/2026-08-10-ch8-editor-feedback-applied.md) | 第8章首次应用编辑反馈的历史记录；其中“已全部落实”的结论已被同日二次复核记录纠正 | **仅用于追溯首次误判和此前第258版，不再作为当前完成依据** |
| [`revisions/2026-07-19-ch8-final-editor-word-delivery.md`](./revisions/2026-07-19-ch8-final-editor-word-delivery.md) | 第8章图8-7/8-8归位、编辑规则全文复核、素材清单同步、Word交付信息和逐页校验 | **使用或重建第8章编辑审阅Word、继续修改第8章前优先读取** |
| [`revisions/2026-07-19-ch8-screenshot-case-correction.md`](./revisions/2026-07-19-ch8-screenshot-case-correction.md) | 第8章图8-7字段映射、图8-8真实连接失败日志、“负责人为空”误判纠正、UTF-8修复和最终版本核验 | **截取图8-7/8-8前优先读取；本文件覆盖旧记录中的失败条件口径** |
| [`revisions/2026-07-19-ch8-figure8-7-8-8-screenshot-cases.md`](./revisions/2026-07-19-ch8-figure8-7-8-8-screenshot-cases.md) | 第8章图8-7、图8-8截图案例 Base、测试记录、失败工作流、截图路径和正文来源信息边界 | **截取图8-7/8-8，或把官方资料转化为读者正文前必读** |
| [`revisions/2026-07-19-ch8-case-dedup-and-quality-audit.md`](./revisions/2026-07-19-ch8-case-dedup-and-quality-audit.md) | 第8章8.5完整案例去重、表格与正文分工、示例Base字段对齐和案例写作硬规则 | **撰写或修改任何章节的完整案例前必读** |
| [`revisions/2026-07-19-ch8-automation-depth-expansion.md`](./revisions/2026-07-19-ch8-automation-depth-expansion.md) | 第8章官方自动化资料核验、深度扩写、图8-4至图8-6来源、截图清单变化和最终校验 | **继续扩写第8章、替换截图或导出Word前必读** |
| `writing-style-analysis.md` | 建委写作风格说明书（给AI的风格Prompt） | **生成任何书稿内容前必读**，确保风格一致 |
| [`feishu-doc-style.md`](./feishu-doc-style.md) | 飞书文档的视觉结构、callout、图表和时间落款偏好 | 创建或改写飞书文档时读取；出版书稿仍以出版规则优先 |
| [`feishu-base-form-experience.md`](./feishu-base-form-experience.md) | 多维表格表单选项表达、二维码图片和 API 能力边界 | 创建或调整飞书表单时读取 |
| `all-docs.md` | 34 篇 AIGC 培训文档的抓取摘要与来源索引（仓库文件约 1 万字符，不是 20 万字全文） | 需要定位历史培训素材和来源文档时读取；不得当作当前事实库 |
| `ch4-editor-feedback.md` | 第4章编辑反馈汇总+修改方案+章节交叉分析 | **改稿前必读**，了解编辑诉求和与其他章节的边界 |
| `ch4-v3-editor-feedback.md` | 第4章 v3 修订稿编辑反馈汇总——标题压缩、出版书面语、图表编号、图文匹配、截图清晰度、术语一致等精修规则 | **第4章二次改稿前必读**，尤其处理截图、图号、读者视角正文时读取 |
| `ch4-v4-revision-log.md` | 第4章 v4 修订稿创建与执行记录——V4 子文档链接、图号体系、已执行校验、后续改图提醒 | **第4章继续审稿/替换图片前必读**，确认 V4 已改范围和图号体系 |
| `interface-screenshot-guidelines.md` | 软件界面截图与图文一致规范——截图格式、核心区域裁切、印刷可读性、标注对应、Word 交稿检查 | **替换图片/审校图文/交稿前必读**，尤其处理软件界面截图清晰度和图文匹配时读取 |
| `ch5-editor-feedback.md` | 第5章编辑反馈去重汇总——书面语、图表规范、操作表述、配套资源统一等全书共性要求 | **写作/改稿时必读**，同步其他作者反馈，避免全书风格和规范不一致 |
| `ch1-editor-feedback-lessons.md` | 第1章编辑反馈汇总（引以为鉴）——术语一致性、图文衔接、逻辑过渡等问题 | **写作时必读**，避免犯同样的错误 |
| [`revisions/2026-07-14-ch2-editor-0713-feedback-applied.md`](./revisions/2026-07-14-ch2-editor-0713-feedback-applied.md) | 第2章 0713 编辑稿的 12 条批注、550 条文本修订、处理决策和校验结果 | **继续修改第2章，或把本轮规则复用到第3、8章前必读** |
| [`revisions/2026-07-14-ch2-parenthetical-numbering-and-word-delivery.md`](./revisions/2026-07-14-ch2-parenthetical-numbering-and-word-delivery.md) | 第2章括号序号纠正、飞书最终版本、Word交付文件信息和逐页校验结果 | **处理列表体例或重新生成第2章Word前必读** |
| [`revisions/2026-07-18-ch3-editor-0713-feedback-applied.md`](./revisions/2026-07-18-ch3-editor-0713-feedback-applied.md) | 第3章41条批注、487处修订、黑白图文、序号、官方核验和Word格式规则 | **继续修改第3章或生成第3章Word前必读** |
| [`revisions/2026-07-18-ch3-full-reaudit-and-figure3-1-correction.md`](./revisions/2026-07-18-ch3-full-reaudit-and-figure3-1-correction.md) | 第3章图3-1画板漏检纠正、41条批注二次复核、修订残留扫描和画板语义校验规则 | **继续修改第3章、处理画板流程图或复核编辑批注前必读** |
| [`revisions/2026-07-18-ch3-word-and-ch8-editor-rule-reaudit.md`](./revisions/2026-07-18-ch3-word-and-ch8-editor-rule-reaudit.md) | 第3章图3-1最终口径与Word交付；第8章结合第2、3章编辑规则的全文复核、截图清单调整和最终校验 | **使用第3章最终Word、继续修改第8章或重截图8-2/8-3前必读** |
| [`revisions/README.md`](./revisions/README.md) | 第2、3、8章和旧第4章相关修订的任务路由、时间线与适用范围 | **修改章节前先读**，再选择与当前章节直接相关的修订，避免一次性加载全部记录 |

## 使用指南

**对于 AI 工具**：
1. 先读本 README 了解项目背景和写作铁律
2. 生成内容前，务必先读 `writing-style-analysis.md` 获取写作风格规范
3. 继续修改具体章节前，先读 `revisions/README.md`，只选择对应章节的修订
4. 涉及飞书功能描述时，必须查阅飞书官方文档验证，禁止凭经验推断
5. 需要定位历史培训素材时再读 `all-docs.md`，不要默认加载

**对于建委**：
- 书稿正文存放在飞书文档中，按章节管理
- `writing-style-analysis.md` 是给其他 AI 工具的「风格说明书」，喂给它就能按你的风格写
- 第4章飞书文档链接：https://my.feishu.cn/wiki/KEKLwUgWHiv9PtkhzXYc6QlnnEk
