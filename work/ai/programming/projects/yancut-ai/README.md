...
- 代码现状：独立源码仓库为私有仓库 `https://github.com/MouMou-Eleven/yancut-ai`；本地 `main` 最新提交 `d5135e9`，远端源码提交 `73085ad`，`upstream` 继续跟踪 `https://github.com/OpenCut-app/OpenCut.git`。本轮新增 GLM 多模态规划和客服人工购买配置；GitHub Actions 工作流因当前令牌缺少 `workflow` 权限，仍只保留在本地历史中。`ai-context` 只保存项目上下文，不保存完整源码。
...
- 2026-09-03 模型与购买流程更新：价格页改为“联系客服购买”，通过客服二维码/联系方式人工确认后开通积分；兼容旧收银台的接口不再创建订单或发起扣款。后台新增客服二维码、客服名称和联系方式配置，未配置时价格页明确提示。GLM-5.3-Flash 多模态规划参数已接入，图片/视频/字幕上下文会进入模型请求。

尚未完成：

- 秒哒生产接线、云端项目同步和跨账号隔离验收；管理后端、积分、订单、订阅与客服人工购买指引已完成本地实现，仍需部署真实 Auth、Postgres、对象存储，并录入真实客服二维码、联系方式和人工开通流程。
