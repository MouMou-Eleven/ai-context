...
- 代码现状：独立源码仓库为私有仓库 `https://github.com/MouMou-Eleven/yancut-ai`；本地 `main` 最新提交 `7295a2b`，远端源码提交 `702ddbc3`，`upstream` 继续跟踪 `https://github.com/OpenCut-app/OpenCut.git`。本轮新增本地免登录演示开关、13 个基础 GPU 特效、扩充贴纸、AI 特效/贴纸命令和共享语言状态；`ai-context` 只保存项目上下文，不保存完整源码。
