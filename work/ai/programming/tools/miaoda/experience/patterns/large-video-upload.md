# 大视频上传决策：按会员配置原生上限，旧分片代理只兜底

> **当前状态：旧方案已被 2026-08-27 的会员分级上传权益取代。** 当前官方计费页明确：免费版项目内单文件 50MB，专业版、旗舰版和企业标准版 500MB。新项目不得默认套用永久分片架构；先确认会员、同步修改前后端限制并测试当前原生上传。本文后半部分保留 [pitfalls.md #14](../pitfalls.md) 在旧环境生产验证过的兼容实现。
>
...
1. 先确认会员：免费版仍为 50MB；需要 50MB 以上文件时使用专业版或以上权益。
2. 在项目对话中明确要求“上传大小改为 500MB，同步修改前后端限制”。
3. 确认对象存储剩余容量能容纳目标文件；500MB 单文件权益不等于有 500MB 剩余存储。
4. 在当前应用、当前后端套餐和正式访问域名下，用无敏感内容、接近真实业务大小的视频测试。
5. 同时检查浏览器网络面板和后端日志：是否出现 CORS、413、函数超时/取消、对象大小不一致。
6. 上传成功后重新加载页面，确认对象仍存在，并测试 `<video>` 首播、拖动进度条和 Range 请求。
7. 如果平台原生上传全部通过，使用当前原生能力。为提高弱网可靠性，可以采用平台或云厂商原生 multipart / 断点续传，最终仍合成为一个正常对象；不使用本文件后半部分的“永久分片 + Range 代理”。
8. 只有旧项目仍复现旧环境三层墙，才启用兼容方案。

“单文件上限提高到 500MB”与“任何网络都无需分片”不是同一件事：前者是付费会员的产品额度；后者是传输可靠性选择。大文件即使能单次上传，移动网络下仍可能更适合可重试的 multipart，但应优先使用存储服务原生 multipart，而不是永久保存业务分片。

## 何时用 / 何时不用

| 场景 | 这个方案 |
|---|---|
| 已按当前会员配置前后端上限，旧项目仍撞 CORS / 413 / supervisor kill | ✅ 用作兼容兜底 |
| 新应用尚未确认会员、对象存储容量和原生上传能力 | ❌ 先配置并测试，不要直接套旧架构 |
...
写提示词的完整模板见 [`prompt-patterns.md` § 大文件上传：先按会员配置 500MB，旧分片代理只兜底](../prompt-patterns.md#大文件上传先按会员配置-500mb旧分片代理只兜底)。
