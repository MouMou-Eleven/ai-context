...
Skill是能力依赖，不是业务项目归属。视频商单仍归对应设计或AI视频项目，科研图按科研交付组织；名称中的“community/社群”不表示某个课程事实属于会员课程。一个主任务可按需要引用多个能力卡，相关领域只保留链接，不复制实体。
...
- 保留LICENSE、NOTICE、依赖锁文件和运行脚本；不入库 `.git/`、环境、缓存、输出、凭据或个人输入。`STRUCTURE.md`仅展开能力条目，不遍历完整外部源码。
