...
**预防**：写脚本时为每个接口列出明确的 required / optional 清单（[case-yungouos-jsapi.md](./cases/yungouos-jsapi.md) 末尾有 5 个接口的完整字段表）。
...
详见 [platform-basics.md](../basics/platform-basics.md) 第七节。
...
## #23 把 Next.js 源码直接导入秒哒 Web 项目 → 构建后黑屏

**实战边界**：2026-08-09「混世魔牛」源码交接中确认。它是当前环境的交付红线，不代表秒哒未来版本永久不支持 Next.js。

**症状**：附件能上传，秒哒也显示创建完成，但 Web 预览或部署页面黑屏；构建链找不到 Next.js 运行时、SSR 入口或 API Routes。

**根因**：当前秒哒 Web 应用使用 React + Vite。Next.js 的服务端渲染、文件路由和服务端 API 依赖与当前 Web 运行时不匹配。

**修复与预防**：交付前把 Web 源码归一为 React + Vite，`npm run build` 输出 `dist/`；提示词明确禁止 `next` 依赖、SSR、Next API Routes 和常驻 Node 服务。每次新项目仍需重新核验当前平台能力。

---

## #24 只上传压缩包但没有把素材解压进对象存储 → 线上 404 或依赖对话附件

**症状**：秒哒能读取源码清单，但部署后图片、视频、音频、图集或模型缺失；刷新、换设备或附件会话失效后资源不可用。

**根因**：对话附件只是交接载体，不是浏览器可长期请求的生产 URL。只保留 ZIP，或者继续使用本机绝对路径、Blob URL、Base64 与临时签名地址，都没有完成云端资源迁移。

**修复与预防**：要求秒哒逐包解压，把静态素材写入云端文件/对象存储，并生成“原相对路径 → 对象键 → 稳定 HTTPS URL → SHA-256”映射；上线前检查 HTTPS、MIME、CORS、音视频 Range、缓存和存储额度。完整处方见 [codex-source-package-deployment.md](./patterns/codex-source-package-deployment.md)。

---

## #25 最终提示词只要求生成应用 → 秒哒不创建产品需求文档

**症状**：秒哒直接开始创建页面或修改代码，但项目中没有正式 PRD，后续迭代缺少统一验收基线。

**根因**：秒哒按明确指令执行；“生成网页应用”不等于“同时生成产品需求文档”。

**修复与预防**：源码、素材和哈希全部校验通过后，最终提示词必须原样包含：
...
## #26 把 Codex 本地浏览器调试流程直接写进秒哒提示词 → 诊断任务无法真实执行

**实战来源**：2026-08-15「混世魔牛」野怪卡死诊断。此前提示词要求通过 URL 查询参数启用诊断，并让秒哒自行完成浏览器验证；建委指出秒哒的开发、构建和发布都在云端，秒哒 AI 不具备 Codex 的浏览器控制和真机复现能力。

**2026-09-25适用范围修订**：以上是当时项目的认识，不再作为平台永久没有浏览器测试能力的断言。言剪曾收到云端 Chromium 通过回执，但用户访问失败，独立复测为白屏且 Vite 核心资源 401，因此该回执不能作为页面可用证据。当前按[运行时边界](../basics/platform-basics.md#运行时诊断的正确边界)核对工具和证据。下文“禁止”限定为没有实际能力/证据却要求或声称完成，以及把云端自报冒充独立浏览器、用户真机或正式发布验收。

**症状**：秒哒回复“已验证”或输出了静态检查结果，但用户在真实页面上仍无法获得可用运行日志；手机端还需要手工修改 URL，或只能看到无法导出的 Console 输出，最终仍不能解释对象为什么突然静止、误判或卡死。

**根因**：把三个不同角色混在了一起：秒哒负责云端源码修改和构建；用户负责在预览版或发布版中真实操作复现；Codex 负责分析运行证据。`?debug=1`、Playwright、浏览器自动点击和真机验证属于特定调试环境或 Codex 的能力，不能默认成秒哒 AI 的执行能力。

**错误做法**：

- 要求秒哒“打开浏览器、自动操作页面、运行 Playwright、确认真机通过”。
- 把 `?enemyTrace=1` 一类 URL 参数设为唯一诊断入口，要求用户手工拼接。
- 只向 Console 高频输出，既无法在手机稳定读取，也可能因逐帧日志进一步制造卡顿。
- 把秒哒的 lint、测试或 build 通过写成“运行时问题已验证解决”。

**正确闭环**：

1. 让秒哒只做窄范围诊断埋点，在当前云端应用中直接显示临时“诊断日志”按钮。
2. 日志进入有上限的内存环形缓冲，记录状态转换、异常事件和 5-10Hz 限频快照，并支持复制全部或下载 JSON。
3. 用户在预览版或发布版中手动复现问题，把日志交给 Codex。
...
## #29 增量包修复了业务代码却漏掉云端构建变量，且跨层假数据掩盖格式错误

**适用**：本地增量包修改了前端认证桥、Edge 与 SQL，云端报告构建和测试通过，但用户点击“获取验证码”失败；云端热修后短信能收到，提交注册又返回固定的数据库业务错误。不限于 Supabase，任何静态前端＋云端构建脚本＋RPC/数据库约束的迁移都应按本条检查。

**来源与状态**：2026-09-26，言剪 AI 百度秒哒 R6。用户上传 R6 后，秒哒 v13 自报 SQL、Edge、构建及浏览器检查通过。用户实际操作发现验证码不发送；云端直接对话定位并修复构建脚本遗漏 `VITE_SUPABASE_URL`／`VITE_SUPABASE_ANON_KEY` 及 `/otp` 空 body 解析。用户随后真实收到短信，但注册显示“注册信息无效”。本地按云端回执哈希复查实物，确认第二个错误仍存在。R6 因此判定未完成，等待 R6.1。

**根因一：云端适配层没有进入增量基线。** 前端 `platform-auth.ts` 读取两个 `VITE_*` 变量，但 R6 没有携带云端专用 `tasks/build-static-preview.sh`，安装合同也没有要求从源码反向枚举变量并逐项核对构建注入。`.env` 中存在变量、TypeScript 通过、Vite 构建成功和登录页可见，都不能证明构建产物拿到了值。配置缺失又被前端捕获成“网络异常”，扩大了误判。

**根因二：两层测试使用了不相同的输入。** Edge 常量 `PLACEHOLDER_HASH` 实际为盐 32 位＋摘要 140 位；SQL 要求 `^[a-f0-9]{32}:[a-f0-9]{128}$`。Deno 测试的数据库替身无条件成功，没有校验传入哈希；PGlite SQL 测试又单独构造了合法 32+128 值。两套测试都绿，但真实 Edge→SQL 组合必然抛 `INVALID_REGISTRATION`。云端 `platform.ts` SHA-256 与本地包相同，排除“云端改坏”猜测。

**为什么云端对话先修好了一段**：秒哒当时能直接读取云端 `.env`、构建脚本和运行日志，因此能快速修复平台专属适配文件；这不证明长提示词比增量包可靠，只说明本地基线缺少该适配层。直接对话只打通验证码请求，没有修复后续 RPC 输入，也没有完成注册闭环。

**防漏规则**：

1. 打包前扫描所有公开构建变量，与示例环境、云端构建脚本、目标产物逐项对照；新增变量必须同时更新三处合同。
2. 配置缺失、网络故障、HTTP 非 JSON 和业务错误分别呈现，禁止统一显示“网络异常”。
3. 关键 RPC 参数复用数据库合同或在测试中执行同一正则/约束；组合测试必须让真实 Edge 参数进入真实 SQL。旧错误值失败、修复值成功都要保留为回归。
4. 浏览器验收不是“页面能打开”，而是完成关键操作并检查请求与副作用。认证批次至少覆盖获取验证码、验证、创建映射、退出、密码重登和找回密码。
5. 用户/秒哒做过云端热修后，下一包先回收热修文件或哈希并合入权威源码；未回收时只允许窄范围合并，不能用旧整文件覆盖。
6. 主成功路径仍失败，继续同一批次补丁，不因“包已上传”或“秒哒已完成”晋级下一批。

本条不表示所有云端变量都应写入 `VITE_*`；只有可公开给浏览器的 URL、anon/public key 才能进入前端构建。service-role、数据库密码、供应商私钥等仍必须仅在服务端保存。统一流程见[构建时公共环境变量与跨层契约](./patterns/codex-miaoda-iterative-increment-workflow.md#构建时公共环境变量与跨层契约)。
