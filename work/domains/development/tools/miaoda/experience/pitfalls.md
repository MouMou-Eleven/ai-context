...
**2026-09-25适用范围修订**：以上是当时项目的认识，不再作为平台永久没有浏览器测试能力的断言。言剪已有云端Chromium测试回执；当前按[运行时边界](../basics/platform-basics.md#运行时诊断的正确边界)核对工具和证据。下文“禁止”限定为没有实际能力/证据却要求或声称完成，以及把云端测试冒充用户真机验收。
