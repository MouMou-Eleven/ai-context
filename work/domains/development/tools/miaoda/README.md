...
- 已有本地项目采用本地修改测试后交付编号 ZIP、用户上传、秒哒应用、回收差异验收的闭环；文本调度不等于附件上传。当前轮未结束不发下一轮；后续增量无需例行重写需求文档或生成无用途 ZIP，见[增量流程](./experience/patterns/codex-miaoda-iterative-increment-workflow.md)。
