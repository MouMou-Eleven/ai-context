...
> 本页记录阶段变化，不能直接用旧实现作为当前能力。当前产品事实从 [`README.md`](./README.md) 指向的适用证据核对，技术边界读 [`architecture-and-upstream.md`](./architecture-and-upstream.md)。

新增重要变化写入对应revision并更新项目索引，本页只保留摘要和链接。下方既有详细阶段记录按历史材料保留，不继续追加为第二套当前状态。

| 日期 | 变化 |
|---|---|
| 2026-09-25 | 按用户要求串行等待，841→886、888→925、927→978、980→1016依次完成；最后独立外部浏览器确认原首页／登录静态预览，后端未接、登录禁用。R4已验包但上传前浏览器自动安全审查拒绝，仍需用户手动上传，未发布。见[迁移回执](./revisions/2026-09-25-miaoda-source-migration.md)。 |
| 2026-09-25 | 平台生成终止期间继续完成R4后台设置/AES/事务与权限本地候选，`fa3e8625`已推送迁移分支；Deno19测试、PGlite27断言通过，尚未上传或云端验收，见[回执](./revisions/2026-09-25-miaoda-source-migration.md)。 |
| 2026-09-25 | 秒哒官方Skill调度、浏览器上传R1/R2/R3与PRD后单次generate-app执行，事件825平台错误；同应用chat恢复最终835同样错误终止，未发布。最新授权平台原生手机号Auth替代旧自建方向，短信/存储/云端冻结构建仍未验收。见[迁移最终回执](./revisions/2026-09-25-miaoda-source-migration.md)。 |
| 2026-09-25 | 修复我的空间对齐、资源标签滚动、参考上传和明确拒绝规则，新增声音配置检查；R1/R2已推送、复建并获秒哒核验。R3原UI运行壳与事务后端候选ba81b374已推迁移分支并上传，类型/测试/独立构建通过；仍有API与旧数据待迁，尚未生成发布。后续目标转为秒哒，不自动继续Vercel发布。见[迁移修订](./revisions/2026-09-25-miaoda-source-migration.md)。 |
| 2026-09-25 | 编辑器统一附件/@，19动效、20真实音效、6转场，修复预览锁和音效插入，实际导出12秒带音轨MP4；MiniMax任务与积分事务落地，详见[工作台修订](./revisions/2026-09-25-editor-voice-integration.md)。随后用户纠正[v1.1声音定价](./revisions/2026-09-25-voice-pricing-v11.md)，撤销误引预估接口，改为实测校准与事后告警暂停。 |
| 2026-09-25 | 普通用户导出发布/另行上传/撤下、真实瀑布流、首页交互去重、模型协议适配与二维码上传；线上Preview验证两轮对话裁切+动效导出10秒MP4。明确Remotion为内部能力，不另开工作台，实际渲染自动回填尚需接线。见[修订](./revisions/2026-09-25-gallery-protocols-rendering.md)。 |
| 2026-09-17 | 补齐本地优先使用手册和浏览器存储边界；工作台素材标签改为单行、AI 快捷指令自动换行、侧栏分组、首页流程与存储提示；审查 Spectrum UI 后确定按需源码参考与能力注册表约束。见 [修订](./revisions/2026-09-17-local-first-manual-spectrum-ui.md)。 |
