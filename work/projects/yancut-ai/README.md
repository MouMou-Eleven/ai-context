...
> 状态：用户已上传并执行 R6；秒哒 v13 回执不能视为认证完成。随后用户通过云端对话修复了静态构建遗漏 `VITE_SUPABASE_URL`／`VITE_SUPABASE_ANON_KEY` 的问题，真实短信已能收到，但注册提交仍返回“注册信息无效”。本地复核确认云端部署的 `platform.ts` 与 R6 包哈希一致，其占位密码哈希为 32+140 位，而 SQL 只接受 32+128 位，必然触发 `INVALID_REGISTRATION`。因此 R6 未完成，须先做 R6.1 补丁并完成真实注册、首管、密码重登和找回密码验收，不能进入 R7；管理员保存、正式域名和 36 个未迁 API 仍未验收。
