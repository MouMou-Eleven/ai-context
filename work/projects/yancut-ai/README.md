...
> 状态：用户已上传 R5，秒哒事件 1709 completed，但其临时 `vitesandbox` 地址被用户确认不可访问；独立 Playwright 复测入口 HTML 200、页面白屏、Vite 核心资源 401，故撤销“渲染通过”口径，仍未正式发布。收到 v10 审查包后，隔离数据库复现身份映射 SQL 的 UUID 函数和 JSON 行转换两处错误。R6 已在本地生成并验证，等待用户上传；真实注册、管理员保存和正式域名仍未验收，36 个 API 仍由云端报告为 503。
...
最新补充：R5 手动上传已完成，事件 1585→1709、版本标签 v12。云端报告 42 项变更、1555 目标哈希与多宽度渲染；但用户访问其 `vitesandbox` 地址失败，独立 Playwright 复测为白屏且 `@vite/client`/HMR 资源 401，因此只保留“云端曾这样报告”的历史，不再写成验收通过。管理员保存未测、36 API 仍 503。用户提供的 v10 包实际只有 8 文件；本地已审查并复现 SQL 成功分支两处错误。R6 已基于实物 hash 生成审查优先增量包，修复 SQL、密码登录安全分流与 refresh token 缺口；不覆盖云端 442 行页面，而是要求按合同小范围合并并回传完整文件。规划 5 个功能批次＋1 轮总验收，实际拆分按反馈调整，不承诺固定上传次数；完整审查与分批范围见[本次修订](./revisions/2026-09-25-miaoda-source-migration.md#r6-本地交付与沙箱渲染口径更正)。
