...
- 当前线上版本：2026-09-25 固定域名指向 READY 生产 `dpl_9TQ2stFmEf1tavWDNeaXESGB9GZT`；最终 Preview 为 `dpl_J5QpJzHunuXRCQw3A224AhdzsLga`，功能代码 `3be68578`＋错误提示补丁 `cb5739d0`。原生产 `dpl_AruyzfJANm5xs1cXw9jNmCLqZ8BS` 保留回退。真实账号、Neon、管理员、积分继续使用；未新增数据库迁移。国内直连此前失败，本轮使用代理验收，不能保证裸连可用。最终承载平台仍为百度秒哒。
...
最新交互与验收见[模式与组合技能升级](./revisions/2026-09-25-composable-skills-ui.md)：11 个可选技能共用目录，首页多选/搜索/@ 引用，模式与技能保存到工程及模型请求。用户文字不被覆盖，工具箱返回沿用草稿；登录注册页居中放大，无协议勾选框。478 项测试通过，真实模型生成两段白板并成功导出 12 秒 MP4；无声短片不等于真人口播质量通过。
...
| AI计划与统一工作台 | 首页模式、11 个可选技能、可编辑提示词、@ 技能与附件、画幅/时长/风格进入持久化需求；每轮最多组合 4 个技能、检索 6 个配方，经过证据/时间/布局校验后转原生命令；云端计划先报价确认，失败退款 |
...
| 最近验证证据 | 2026-09-25 本轮：Bun 478 项、4194 次断言通过；TypeScript、正式构建、GitHub三平台CI通过，五页面五宽度无横溢；组合技能/附件/工程保存实测通过；真实模型两段白板计划→执行→导出12秒MP4通过。此前9:16创建、素材重新关联、关键帧、13秒1080p白板记录作为历史证据保留 |
...
| [`revisions/2026-09-25-composable-skills-ui.md`](./revisions/2026-09-25-composable-skills-ui.md) | 最新迭代：模式与多选技能、统一提示词、响应式UI、注册协议、真实模型组合白板及MP4验收 |
