...
> 当前口径确认：2026-09-15；历史通过记录不等同于现行线上验收

个人测试入口：[在线工作台](https://yancut-ai-personal.vercel.app/studio/workbench) · [填写本人 GLM Key](https://yancut-ai-personal.vercel.app/ai-settings)。无需启动本地服务；项目仍存当前浏览器。部署与测试范围见 [本轮记录](./revisions/2026-09-15-personal-vercel-testing.md)。
...
- 源码仓库为私有仓库 `https://github.com/MouMou-Eleven/yancut-ai`，`upstream` 跟踪 `https://github.com/OpenCut-app/OpenCut.git`。09-15 核对时本地 HEAD 仍为 `c10df52`，另有多轮未提交实现；不能把旧提交称为包含当前全部功能。`ai-context` 保存项目上下文，不保存完整源码。
...
以下结合 09-10、09-15 修订及本轮源码检查更新；历次证据见 [history.md](./history.md)。
...
| 模板与执行稳定性 | 4 个场景入口：口播、字幕、混剪、动效；6 个内部技能合同。项目指纹、串行命令、严格保存与撤销/重做；中途失败停止并保留已完成修改，不保证整轮原子回滚 |
| 专业剪辑 | 已接入停顿处理、字幕、倍速与音调保持、淡入淡出、转场、ducking、响度估计、代理、高光与速度曲线路径；复杂与长视频场景仍待系统验证 |
| 包装与关键帧 | 原生文字动效与数值排行、现有文字定点修订、时间/字幕锚点、保守安全区检查；支持手动关键帧。尚无人物脸部跟踪和任意 AE 效果生成 |
...
| 最近验证证据 | 09-10 合成原片 90 秒→15 秒分镜及导出；09-15 本轮 141 项相关测试、901 次断言、TypeScript、Vercel 构建通过，并在线导入/手动分割/导出 15 秒 H.264/AAC。本人有效 Key 的在线规划仍待实测；不证明真人口播质量或长视频性能 |
...
*索引与当前口径更新：2026-09-15；部署状态单独记录，不能从本地测试推断。*
