...
> 状态：用户已上传 R5，秒哒事件 1709 completed，回执报告品牌与静态入口已更新，未正式发布。收到 v10 审查包后，隔离数据库复现身份映射 SQL 的 UUID 函数和 JSON 行转换两处错误；真实注册与管理员保存尚未验收。36 个 API 仍由云端报告为 503。最新要求为手机号＋密码登录、手机号＋密码＋验证码注册；R6 尚未交付，先回收已生成的 R5 审查包，恢复本地测试后交付、用户逐批上传的方式。
...
最新补充：R5 手动上传已完成，事件 1585→1709、版本标签 v12。云端报告 42 项变更、1555 目标哈希、公开品牌 GET 和多宽度渲染通过，但管理员保存未测、36 API 仍 503。用户提供的 v10 包实际只有 8 文件；本地已审查并复现 SQL 成功分支两处错误，不再沿用“安全回执通过等于认证完成”。云端 R5 修改保留的 442 行手机号页面不在 v10 包内，须回收 `r5-branding-review-v11.zip` 后合入本地再制作 R6。规划 5 个功能批次＋1 轮总验收，实际拆分按反馈调整，不承诺固定上传次数；完整审查与分批范围见[本次修订](./revisions/2026-09-25-miaoda-source-migration.md#r5-回执与-v10-实物审查用户再次确认增量交付)。以下早前记录保留执行过程，R5“待上传”状态已由本段取代。
...
R4 源码 `fa3e8625` 已推送迁移分支并应用到秒哒源码树：后台设置、AES 兼容和事务权限门控通过 Deno 19 测试及隔离 SQL 27 断言；应用源码时未执行 SQL、未接通接口、未重建静态前端，不能据此认定后台可用。新一轮双配色 Logo、公共品牌配置与管理员上传／恢复入口已打为 R5，源码 `0b0cb006` 及后续文档 `b74e3395` 均已推送迁移分支并远端核实。网站配置 Deno 23 项、隔离 SQL 15 断言通过；R5 独立恢复目录完成 1555 文件哈希、冻结安装、TypeScript 和 Vite 3105 模块构建。管理交互为本地模拟 API 验证，包仍待手动上传，详见[迁移回执](./revisions/2026-09-25-miaoda-source-migration.md)。
