...
- 当前线上版本：2026-09-25 固定域名指向 READY 生产 `dpl_CNevkuCdVm2cnLAZjGTLSvwGinxr`；验收 Preview 为 `dpl_2JaQ1L4oWk1aBuMWbj533BzXuMxs`，运行代码 `ef7f877c`。前版 `dpl_9TQ2stFmEf1tavWDNeaXESGB9GZT` 保留回退。真实账号、Neon、管理员、积分继续使用；未新增数据库迁移。国内直连此前失败，本轮使用代理验收，不能保证裸连可用。最终承载平台仍为百度秒哒。
...
| 声音与渲染服务 | 声音登记/克隆/合成接口边界和任务归属已实现；Remotion Worker已本机真实出片，公网部署及AI结果自动回填尚未完成，HyperFrames独立HTML Worker未实现；已有原生动效导出可先用于最小闭环 |
...
| 最近验证证据 | 2026-09-25 本轮：Bun 497 项、4282 断言、TypeScript、正式构建和三平台CI通过；真实发布/撤下、归属校验、分页、私有Blob二维码验收通过；Preview两轮真实模型完成裁切+动效并导出10秒MP4。此前白板/口播合成测试留作历史证据，不等于真人长视频质量通过 |
