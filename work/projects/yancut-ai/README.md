...
> 状态：云端测试版已上线，进入真实账号与素材验收阶段
> 当前口径确认：2026-09-25；历史通过记录不等同于现行线上验收

个人在线入口：[言剪 AI](https://yancut-ai-personal.vercel.app) · [登录/注册](https://yancut-ai-personal.vercel.app/login) · [在线帮助](https://yancut-ai-personal.vercel.app/studio/help)。当前使用真实账号、Neon 数据库和私有 Blob；首次注册管理员需要本机私密建站口令。云端上线记录见 [2026-09-25 云端上线验收](./revisions/2026-09-25-online-cloud-launch.md)；旧的本地优先说明和个人 Key 说明只作为历史记录保留。
...
- 云端测试版：2026-09-25 已将固定域名 `https://yancut-ai-personal.vercel.app` 指向通过构建的生产部署 `dpl_7Mb557jy4H8hKGAqGkTabbR8H1cs`；Preview 验收部署为 `dpl_5vaxnYqP4h7A1V3yxC3eA9Qjwh4E`，源码提交为 `bcfad5a4`。线上使用真实账号、Neon PostgreSQL、私有 Vercel Blob、管理员配置和积分扣费；不再把“无需登录、工程只在浏览器、个人 Key”当作当前产品入口。最终承载平台仍为百度秒哒。
...
| 能力 | 当前实现与验证边界 |
|---|---|
| 中文剪辑工作台 | 基于OpenCut真实素材、预览、时间线与导出；云端账号保存工程与素材，浏览器负责预览、解码和当前 MP4 导出；主要界面支持中英双语 |
| AI计划与统一工作台 | 项目需求、素材、画面证据、可调整分镜进入同一时间线；云端计划先报价、确认再扣积分，失败退款，不把本地规则当作模型结果 |
...
| 最近验证证据 | 2026-09-25：Bun 全量 391 项通过、3928 次断言；TypeScript、本地构建、Vercel Preview 构建通过；固定域名登录、注册表单、健康接口、云端状态和未登录 401 已验收 |
| 豆包 ASR 2.0 标准版 | 已接入服务端异步提交/查询、300 积分/小时报价与确认扣费、失败退分、云端任务归属和刷新恢复；真人长视频、上传速度和供应商等待时长仍待用户实测 |
| 当前录屏验收 | 云端账号制版本已上线；口播精剪＋字幕＋包装的真人素材闭环需用户首次注册后继续验收，不能用小型合成片替代真人质量判断 |
