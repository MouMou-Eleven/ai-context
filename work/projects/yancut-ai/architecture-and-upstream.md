...
> 上游基线不变；当前流程与部署边界修订日期 2026-09-15。更早上游版本表是当时的核验记录，不作为今日最新版。
...
- 模型输出先展示计划，用户确认后再执行。
- 本地演示允许明确标注的规则规划器；个人 Key 测试模式只返回真实模型结果或明确错误，不静默改用规则。
- 每份计划绑定项目内容指纹；执行时项目已变化则拒绝旧计划。
- 执行前显示能力预检，不允许把未接入或未配置的服务静默伪装为可用。
- 执行后生成有限长度回执；中途失败停止并保留已完成修改，用户可撤销；严格保存失败单独报告，不保证整轮自动回滚。
...
- 画面能力还包括 13 个基础 GPU 特效、7 个模糊变体和可由 AI 调用的贴纸命令；本地演示通过 `YANCUT_LOCAL_DEMO_MODE=true` 提供未登录的计划与积分环境，公网部署必须关闭。
...
2026-09-15 个人测试模式新增 `YANCUT_BYOK_ENABLED=true`：用户在 `/ai-settings` 填 Key，仅存标签页会话，请求头传至本站后转发智谱。此模式与商业后台分开，不开放本地演示身份、钱包、管理接口或云渲染。Vercel 承载网站与模型请求；工程仍在浏览器，后续稳定才迁移百度秒哒。
...
| 言剪 AI 源码仓库 | 私有 `MouMou-Eleven/yancut-ai`；本地 `main` 提交 `7295a2b`；远端提交 `702ddbc3`；`upstream` 保留 OpenCut | 本地 Git、GitHub API、Git Data API 远端核验 | 2026-09-03 |
