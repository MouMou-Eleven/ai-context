...
try {
    ...
    if ($desktopHash -ne $repoHash) {
        throw "Desktop HTML does not match repository navigation/STRUCTURE.html: $targetFullPath"
    }
    ...
    Write-Host "Repository system/repository/navigation/STRUCTURE.html is current; a later Git hook or manual sync will retry the desktop mirror."
    return
}
