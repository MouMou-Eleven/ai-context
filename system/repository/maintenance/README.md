...
导航中文说明为必填：日常知识条目逐项登记具体用途，标题与说明分开，不能留空、仅重复标题或使用占位语。校验器会阻止缺少登记的新增条目；完整文件视图还检查每个目录与文件都有中文说明，快照内部和技术文件可按类型生成。桌面与窄屏均显示说明，不再按标题相同或屏幕尺寸隐藏。脚本只能查形式完整性，维护AI仍须核对说明是否准确描述正文。
