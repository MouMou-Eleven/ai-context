...
参考库较大时用可选readWhen逐项登记whenAny和paths，也可加excludeAny／intents；仅在方法已采用且场景条件命中后加入正文计划，未采用、查询或排除任务不会加载。方法README同时提供按场景的参考表，供网页AI与没有命中关键词时使用；实际材料的语义仍由AI核对，不盲读整个库。新增模板映射要同时验证命中、无关场景隔离与路径存在。
