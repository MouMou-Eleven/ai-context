...
| CC Switch | 3.18.0 |
| iKuuuVPN | 0.17.5+1；2026-09-07 实测本地代理可访问 GitHub 网页和 API |
| Clash for Windows | 0.20.16.3-ikuuu；持久配置为规则模式，国内域名和中国大陆 IP 直连，GitHub 走主选择组 |
...
| 磁盘状态 | 2026-09-24 再扫四盘并清理已批准的 Codex 工作产物、AE、360 与剪映缓存；当次容量、保留项与后续定位顺序见磁盘专文，历史空闲量不能作为当前读数 |
...
- Codex 会话 Junction 仍指向 F 盘；`Documents\Codex` 的任务生成物独立占用 C 盘，删聊天不会自动删工作文件。不能仅凭数据库未索引就判为无用。本轮五个工作目录是按建委明确批准的编号清理，编号 5 保留。
- E 盘 AE `.aecache` 再次增长到约 22.39 GiB，360 离线缓存约 1.94 GiB；均已按停进程、固定路径、逐文件验证处理。C 盘剪映固定缓存约 1.18 GiB 已在关闭后清理，锁定项留存。微信、剪映草稿和用户资料不做自动批删。
- 下次请求清盘，直接从 [`disk-cleanup-and-codex-storage.md`](./disk-cleanup-and-codex-storage.md) 顶部的 2026-09-24 复用入口开始，先量当前值、再按 A/B/C 闸门执行；不要重复无差别全盘检索，也不要把当次空间数字当永恒值。
...
*文件最后整理：2026-09-24；存储事实最后核验：2026-09-24；其余软件与网络事实按各节原核验日期理解。*
